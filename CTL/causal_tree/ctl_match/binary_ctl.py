...
class CTLMatch(CausalTree):
    ...
    def _eval_fast(self, train_x, train_y, train_t, val_x, val_y, val_t, unique_vals, col):
        ...
        train_scaled_x = self.normalizer.transform(train_x)
        val_scaled_x = self.normalizer.transform(val_x)
        ...
        # TODO: only distance for estimation set?
        ...
        above_centroid_avg_dist = np.sqrt(np.sum((above_treated_centroid - above_control_centroid) ** 2, axis=-1))
        ...
        below_centroid_avg_dist = np.sqrt(np.sum((below_treated_centroid - below_control_centroid) ** 2, axis=-1))

        if len(above_centroid_avg_dist) > 1:
            above_penalty = (above_centroid_avg_dist - np.min(above_centroid_avg_dist) + 1) / (
                    np.max(above_centroid_avg_dist) - np.min(above_centroid_avg_dist) + 1)
            above_penalty = (above_penalty + 1.0) / 2
        else:
            above_penalty = 1.0
        if len(below_centroid_avg_dist) > 1:
            below_penalty = (below_centroid_avg_dist - np.min(below_centroid_avg_dist) + 1) / (
                    np.max(below_centroid_avg_dist) - np.min(below_centroid_avg_dist) + 1)
            below_penalty = (below_penalty + 1.0) / 2
        else:
            below_penalty = 1.0

        train_upper_mse = (1 - self.weight) * train_upper_effect ** 2
        upper_cost = self.weight * np.abs(train_upper_effect - val_upper_effect) * above_penalty
        ...
        lower_cost = self.weight * np.abs(train_lower_effect - val_lower_effect) * below_penalty
